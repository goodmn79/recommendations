...
public final class SQLQuery {
    ...
    public  static final  String USER_OF = "SELECT COUNT(*) > 0 FROM TRANSACTIONS t JOIN USERS u ON u.ID = t.USER_ID JOIN PRODUCTS p ON p.ID = t.PRODUCT_ID WHERE USER_ID = ? AND p.TYPE = ?";
    public  static  final String ACTIVE_USER_OF = "SELECT COUNT(*) >= 5 FROM TRANSACTIONS t JOIN USERS u ON u.ID = t.USER_ID JOIN PRODUCTS p ON p.ID = t.PRODUCT_ID WHERE USER_ID = ? AND p.TYPE = ?";

    // через String.format() подставляем comparisonOperator %s этот оператор сравнения передается как строка, constant %d передается как число. Необходима проверка через matches()
    public  static  final String TRANSACTION_SUM_COMPARE = "SELECT EXISTS (SELECT 1 FROM TRANSACTIONS t JOIN USERS u ON u.ID = t.USER_ID JOIN PRODUCTS p ON p.ID = t.PRODUCT_ID WHERE t.USER_ID = ? AND p.TYPE = ? AND t.TYPE = ? GROUP BY u.ID HAVING SUM(t.AMOUNT) %s %d)";
    // через String.format() подставляем comparisonOperator %s. Необходима проверка через matches()
    public  static  final String TRANSACTION_SUM_COMPARE_DEPOSIT_WITHDRAW  = "SELECT EXISTS (SELECT 1 FROM TRANSACTIONS t JOIN USERS u ON u.ID = t.USER_ID JOIN PRODUCTS p on p.ID = t.PRODUCT_ID WHERE USER_ID = ? AND p.TYPE = ? GROUP BY u.ID HAVING SUM(CASE WHEN t.TYPE = 'DEPOSIT' THEN t.AMOUNT ELSE 0 END) %s SUM(CASE WHEN t.TYPE = 'WITHDRAW' THEN t.AMOUNT ELSE 0 END))";
