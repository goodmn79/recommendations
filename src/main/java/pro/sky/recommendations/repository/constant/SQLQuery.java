...
public final class SQLQuery {
    ...
    public  static final  String USER_OF = "SELECT COUNT(*) > 0 FROM TRANSACTIONS t JOIN USERS u ON u.ID = t.USER_ID JOIN PRODUCTS p ON p.ID = t.PRODUCT_ID WHERE t.USER_ID = ? AND p.TYPE = ?";
    public  static  final String ACTIVE_USER_OF = "SELECT COUNT(*) >= 5 FROM TRANSACTIONS t JOIN USERS u ON u.ID = t.USER_ID JOIN PRODUCTS p ON p.ID = t.PRODUCT_ID WHERE t.USER_ID = ? AND p.TYPE = ?";
    public  static  final String TRANSACTION_SUM_COMPARE = "SELECT EXISTS (SELECT 1 FROM TRANSACTIONS t JOIN USERS u ON u.ID = t.USER_ID JOIN PRODUCTS p ON p.ID = t.PRODUCT_ID WHERE t.USER_ID = ? AND p.TYPE = ? AND t.TYPE = ? GROUP BY u.ID HAVING SUM(t.AMOUNT) ? ?)";
